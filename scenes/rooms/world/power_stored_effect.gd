...
func _ready() -> void:
	...
	#position += Vector2(randi_range(60,120),randi_range(-10,10))
	tween.tween_property(self, "position:y", position.y-40, 1).set_trans(Tween.TRANS_CUBIC)
...
func set_value(value: float) -> void:
	power.text = str(snapped(value,0.01))
